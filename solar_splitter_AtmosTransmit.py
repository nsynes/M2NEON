...
output solar radiation rasters for half-monthly periods. This script incorporates atmospheric transmittance into the solar radiation (from Frank's calculations) on a daily basis.
...
dirOut = r"C:\Dropbox (ASU)\M2NEON\Paper_2\DATA\Solar%s_%s" %(Site, RasterModel)
dirRaster = r"C:\Dropbox (ASU)\M2NEON\Paper_2\DATA\RASTER"
pthSensorPoints = r"C:\Dropbox (ASU)\M2NEON\DGPS\CleanedLinked\%s_Point_Sensors.shp" %Site
df = pandas.read_csv(r"C:\Dropbox (ASU)\M2NEON\Paper_2\ANALYSIS\AtmosphericTransmittance\%s_%s.csv" %(Site, Year), index_col="yday")
